...
def convert_to_fsh(processed_xlsforms):
    ...
    with tqdm(total=len(processed_xlsforms), desc="Converting to FSH", dynamic_ncols=True) as pbar:
        for file_name, df_survey, df_choices, short_name, short_id, version, title, lpds_lpds_healthboard_abbreviation in processed_xlsforms:
                logging.info(f'Converting {file_name}...')
                questionnaire_fsh_lines = create_fsh_questionnaire(df_survey, short_name, short_id, version, title, lpds_lpds_healthboard_abbreviation)            
                questionnaire_terminology_fsh_lines = create_fsh_questionnaire_terminology(df_choices, short_name, short_id, version, title, lpds_lpds_healthboard_abbreviation)
                ## AT 15/11/2023: decided in Oct to (temporary) not use this feature until a proper use case for it has been identified. 
                # question_reference_codesystem_fsh_lines, code_tuple = create_fsh_question_reference_codesystem(df_survey, short_name, short_id, version, title, lpds_lpds_healthboard_abbreviation)
                ...
                pbar.update(1)
                fsh_lines_list.append((file_name, questionnaire_fsh_lines, questionnaire_terminology_fsh_lines, short_name, version))
